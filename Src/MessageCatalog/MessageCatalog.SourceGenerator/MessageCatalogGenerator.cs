    ...
    public class MessageCatalogGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(messagesProvider, (spc, item) =>
            {
                ...
                var messageItemSource = GenerateMessageItem(categories, servities, classPrefix);
        ...
        private string GenerateMessageItem(List<string> categories, List<string> servities, string classPrefix)
        {
            var categoryMembers = string.Join(",\n        ", categories);
            var servityMembers = string.Join(",\n        ", servities);
            ...
            var servityEnumName = string.IsNullOrEmpty(classPrefix) ? "DefaultMessageServity" : $"{classPrefix}MessageServity";
            ...
            return $$"""
                ...
                namespace MessageCatalog.Core
                {
                    ...
                    public class {{itemClassName}}
                    {
                        ...
                        public {{itemClassName}}(string id, string text, {{categoryEnumName}} category, {{servityEnumName}} servity, string description, System.Func<string, string> textResolver)
                        {
                            _id = id;
                        ...
                        /// Message severity
                        /// </summary>
                        public {{servityEnumName}} Servity { get; } = {{servityEnumName}}.None;
        ...
        private string GenerateMessageCatalog(MessageDefinition[] messages, string classPrefix, string tsvFileName)
        {
            ...
            var initializations = string.Join("\n            ", messages.Select(m =>
            {
                ...
                return $"{m.Id} = new {itemClassName}(\"{m.Id}\", \"{EscapeString(m.Text)}\", {categoryEnumName}.{category}, {servityEnumName}.{severity}, \"{description}\", _textResolver);";
            }));
